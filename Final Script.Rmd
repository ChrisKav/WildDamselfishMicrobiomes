...
class.barplot <- plot_bar(selected.class, "Location", fill="Phylum") + facet_wrap(Treatment~FishSpecies, scales= "free_x", nrow=2)

```

```{r echo=TRUE, include=TRUE, tidy=TRUE}
ps.class <- tax_glom(ps.ra, taxrank="Class", NArm=FALSE)
ps.class <- transform_sample_counts(ps.class, function (x) sqrt(sqrt(sqrt(x))))

selected.class <- subset_taxa(ps.class, Class=="Bacteroidia" | Class == "Planctomycetia" | Class == "Clostridia"
                              | Class == "Gammaproteobacteria" | Class ==  "Deltaproteobacteria" | Class == "Mollicutes" | 
                                Class == "Flavobacteriia" | Class == "Erysipelotrichi" | Class == "Cytophagia")


plot_bar(ps.ra, "FishSpecies", fill="Phylum") + facet_wrap(Treatment~Location, scales= "free_x", nrow=2)

plot_bar(selected.class)
plot_bar(selected.class, fill="Class")
plot_bar(selected.class, x="FishSpecies", fill="Class")
plot_bar(selected.class, "Class", fill="Phylum", facet_grid=Treatment~Location)
plot_bar(selected.class, "Phylum", fill="Phylum", facet_grid=Treatment~Location)
plot_bar(ps.class, "Phylum", fill="Phylum", facet_grid=Treatment~Location)
...
plot_bar(ps.rarefied, fill="Class")
plot_bar(ps.rarefied, fill="Phylum") + facet_wrap(~Location, scales="free_x", nrow=1)

plot_richness(ps.rarefied, x="Location", measures=c("Observed", "Shannon")) + geom_boxplot()
plot_richness(ps.rarefied, x="FishSpecies", measures=c("Observed", "Shannon")) + geom_boxplot()
plot_richness(ps.rarefied, x="Treatment", measures=c("Observed", "Shannon")) + geom_boxplot()

sig.loc.class <- subset(otu_table(ps.class), rownames(otu_table(ps.class)) %in% row.names(sig.loc.class))
